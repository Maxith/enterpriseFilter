var popup = {
	...
	initData : function (){
		_this = this;
		//读取保存状态
		this.sendMessageBack('isInitData',{}, function(response){
			if(response.status != 200){
				// 初始化数据
				_this.sendMessageBack('saveList', {'message': blackList}, function(response){
					if(response.status == 200){
						_this.showList(response.data);
						// 通知background，修改状态
						_this.sendMessageBack('updateStatus');
					}
				});
			}
		});
